...
server.tool(
  'send_file',
  `Send a file to a channel as an attachment. Use this for reports, HTML pages, CSVs, or any content better delivered as a file than inline text.

The file content is passed as a string — generate the content first, then call this tool.

By default sends to the current group's channel. Main group can target a different channel with target_jid (use the channel's JID from registered_groups, e.g. "slack:C012AB3CD").`,
  {
    content: z.string().describe('The file content as text'),
    filename: z
      .string()
      .describe(
        'Filename including extension (e.g., "report.html", "data.csv")',
      ),
    comment: z
      .string()
      .optional()
    ...
    target_jid: z
      ...
      .optional()
      .describe(
        '(Main group only) JID of the channel to send to. Defaults to current group.',
      ),
  },
  async (args) => {
    ...
    return {
      content: [{ type: 'text' as const, text: 'File queued for delivery.' }],
    };
  },
);
